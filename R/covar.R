...
#'
#'  COV is calculated by dividing the between-group standard deviation (BGSD)
#'  by the setting average, multiplied by 100. BGSD is calculated as the square
#'  root of the weighted average of squared differences between the subgroup
#'  estimates and the setting average. Squared differences are weighted by each
#'  subgroup’s population share. For more information on this inequality measure
#'  see Schlotheuber (2022) below.
#'
...
#'  original dataset. COV is calculated for each of the simulated sample
#'  datasets. The 95% confidence intervals are based on the 2.5th and 97.5th
#'  percentiles of the COV results. See Ahn (2019) below for further
#'  information.
#'
#'  **Interpretation:** COV only has positive values, with larger values
#'  indicating higher levels of inequality. COV is 0 if there is no
#'  inequality. COV has no unit.
#'
...
covar <- function(est,
  ...
  if (!any(is.na(se)) & !is.null(se) &
    ...
    for (j in 1:sim) {
      ...
      covar_sim[j] <- with(simulated_data,
