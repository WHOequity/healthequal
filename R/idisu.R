...
#'  original dataset. IDISU is calculated for each of the simulated sample
#'  datasets. The 95% confidence intervals are based on the 2.5th and 97.5th
#'  percentiles of the IDISU results. See Ahn (2019) below for further
#'  information.
#'
#'  **Interpretation:** IDISU has only positive values, with larger values
#'  indicating higher levels of inequality. IDISU is 0 if there is no
#'  inequality. IDISU has no unit.
#'
...
idisu <- function(est,
  ...
  if (!any(is.na(se)) & !is.null(se) &
    ...
    for (j in 1:sim) {
      ...
      idisu_sim[j] <- with(simulated_data,
                           ((100 / n) * sum(abs(simulation - mean_sim))
                            / mean_sim))
    }
