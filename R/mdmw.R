...
#'  absolute differences between the subgroup estimates and the mean.
#'  Absolute differences are weighted by each subgroup's population share. For
#'  more information on this inequality measure see Schlotheuber (2022) below.
#'
...
#'  original dataset. MDMW is calculated for each of the simulated sample
#'  datasets. The 95% confidence intervals are based on the 2.5th and 97.5th
#'  percentiles of the MDMW results. See Ahn (2019) below for further
#'  information.
#'
#'  **Interpretation:** MDMW only has positive values, with larger values
#'  indicating higher levels of inequality. MDMW is 0 if there is no
#'  inequality. MDMW has the same unit as the indicator.
#'
...
mdmw <- function(est,
  ...
  if (!any(is.na(se)) & !is.null(se) &
    ...
    for (j in 1:sim) {
      ...
      mdmw_sim[j] <- with(simulated_data,
                          sum(popsh * (abs(simulation - weighted_mean_sim))))

    }

    boot.lcl <- quantile(mdmw_sim,
                         probs = c(0.025),
                         na.rm = TRUE,
                         names = FALSE)
    boot.ucl <- quantile(mdmw_sim,
                         probs = c(0.975),
                         na.rm = TRUE,
                         names = FALSE)
  }
